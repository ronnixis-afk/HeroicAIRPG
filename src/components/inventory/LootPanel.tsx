...
const LootItemDisplay: React.FC<{ item: Item }> = ({ item }) => (
    <div className="bg-brand-primary/20 p-4 rounded-2xl flex justify-between items-center border border-brand-surface shadow-inner group hover:bg-brand-primary/40 transition-all">
        ...
        {item.price !== undefined && item.price > 0 && (
            <div className="text-body-sm font-bold text-brand-accent flex items-center gap-1.5 flex-shrink-0 bg-brand-accent/5 px-2.5 py-1 rounded-lg border border-brand-accent/20 shadow-sm leading-none">
            ...
            <div className="space-y-8 py-2 animate-fade-in">
                {lootState.isLoading ? (
                    <div className="flex flex-col items-center justify-center min-h-[300px] space-y-4">
                        ...
                        <p className="text-body-base text-brand-text-muted font-bold">Generating Loot...</p>
                    </div>
                ) : (
