...
export const WizardNavigation: React.FC<WizardNavigationProps> = ({ onBack, onNext, nextLabel, isNextDisabled, showBack }) => {
    return (
        <div className="pt-8 flex items-center justify-center gap-4 flex-shrink-0">
            <button onClick={onBack} className={`btn-secondary btn-md rounded-xl px-8 ${!showBack ? 'invisible' : ''}`}>
                Back
            </button>
            <button
                onClick={onNext}
                disabled={isNextDisabled}
                className="btn-primary btn-md flex-1 rounded-xl shadow-xl shadow-brand-accent/20 flex items-center justify-center gap-3"
            >
                {nextLabel}
